...
class LearningAlgorithm():
	def __init__(self):
		...
		acc, acc_op = tf.metrics.accuracy(labels=tf.round(y_), predictions=tf.round(y2))
		...
		while True:
			if batchesLeft > 0:
				if i % batch_size == 0:
					...
					out_batch, _ = sess.run((y2, train_step), feed_dict={x_: batch_x, y_: batch_y})
			else:
				batchesLeft = int(len(datasetTargets)/batch_size)
				i = -1
			...
			# Print Test:
			if k % 1000 == 0:
				v = sess.run([acc, acc_op], feed_dict={y2: out_batch, y_: batch_y})					
				print("Accuracy:")
				print(v[0])
